...
export default function Banner({ icon, color, title, description }: BannerProps) {
    ...
    const { bg, border } = getColorClasses(color);

    const glassClasses = `backdrop-blur-lg transition-all duration-300 ease-in-out`;
    const blendedBackground = `${bg} bg-opacity-10`;

    return (
        <Box className={`flex w-full flex-col md:flex-row items-start md:items-center mt-2 md:mt-4 py-2 px-4 md:px-2 md:space-x-2 lg:space-x-4 rounded-xl border-l-8 ${glassClasses} ${border} ${blendedBackground}`}>
